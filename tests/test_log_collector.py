...
from src.vtherm_api.log_collector import (
    VThermLogEntry,
    VThermLogHandler,
    VThermLogger,
    get_vtherm_logger,
...
class TestExtractThermostatHint:
    ...
    def test_standard_pattern(self):
        assert _extract_thermostat_hint(
            "Salon - Temperature changed to 21.5°C") == "Salon"

    def test_pattern_with_spaces(self):
        assert _extract_thermostat_hint(
            "Bureau Étage - HVAC mode is heat") == "Bureau Étage"
...
class TestVThermLogHandler:
    ...
    def test_filter_by_thermostat_with_prefix(self):
        ...
        handler.emit(self._make_record(
            "VersatileThermostat-Salon - msg from thermostat"))
        handler.emit(self._make_record(
            "VersatileThermostat-Salon-input_boolean.heater1 - msg from underlying"))
    ...
    def test_filter_by_thermostat_with_various_prefixes(self):
        ...
        handler.emit(self._make_record(
            "SafetyManager-Multi-switch - checking safety"))
        handler.emit(self._make_record(
            "WindowManager-Multi-switch - Window auto is on"))
        handler.emit(self._make_record(
            "HeatingFailureDetectionManager-Multi-switch - disabled"))
        handler.emit(self._make_record("Bureau - should not match"))
        ...
        assert any(
            "HeatingFailureDetectionManager-Multi-switch" in m for m in messages)
    ...
    def test_filter_by_thermostat_no_false_positive_with_prefix(self):
        ...
        handler.emit(self._make_record(
            "VersatileThermostat-Multi-switch - msg2"))
    ...
    def test_periodic_purge_in_emit(self):
        ...
        assert all(
            "old" != e.message for e in entries if e.thermostat_hint is None)
...
class TestAsyncExportLogs:
    ...
    async def test_export_creates_file_and_notification(self, _mock_sign, tmp_path):
        ...
        hass.async_add_executor_job = AsyncMock(
            side_effect=lambda fn, *a: fn(*a) if not a else fn())
