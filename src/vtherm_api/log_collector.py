...
class VThermLogger(logging.Logger):
    ...
    # set by VThermLogHandler.__init__
...
async def async_register_log_download_endpoint(hass: HomeAssistant) -> None:
    ...
    class LogDownloadView(HomeAssistantView):
        ...
        async def get(self, request: web.Request, filename: str) -> web.Response:  # pylint: disable=unused-argument
            ...
            if not re.match(r"^vtherm_logs_[a-z0-9_]+_\d{8}_\d{6}\.log$", filename):
                _LOGGER.warning(
                    "Attempted to download invalid log filename: %s", filename)
            ...
            try:
                ...
                _LOGGER.warning(
                    "Attempted to download file outside log directory: %s", filepath)
                return web.Response(status=400, text="Invalid path")

            if not filepath.exists():
                _LOGGER.warning("Log file does not exist: %s", filepath)
                _LOGGER.debug("Directory contents: %s", list(log_dir.glob(
                    "*.log")) if log_dir.exists() else "Directory doesn't exist")
            ...
            try:
                ...
                _LOGGER.warning(
                    "Log file was deleted since existence check: %s", filepath)
                return web.Response(status=404, text="Log file not found")
            except OSError as err:
                _LOGGER.error(
                    "OS error serving log file %s: %s", filepath, err)
                return web.Response(status=500, text=f"OS error: {err}")
            except Exception as err:  # pylint: disable=broad-except
                _LOGGER.error(
                    "Unexpected error serving log file %s: %s", filepath, err)
    ...
    if hass.http is not None:
        ...
        _LOGGER.warning(
            "HTTP server not available, log download endpoint not registered")
...
async def async_export_logs(
    ...
    header = _format_header(label, eff_start, eff_end,
                            log_level.upper(), len(entries), config_entry)
