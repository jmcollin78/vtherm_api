...
| ----------------------------------------- | -------------------------------------------------------------------- | ---------- |
| **Phase 1** — `vtherm-api` (contrat)      | Interface factory + registre + extension runtime + tests + version   | ✅ **Fait** |
| **Phase 2** — Cœur `versatile_thermostat` | Retraits auto-fan + chargement des managers externes + accès runtime | ⬜ À faire  |
| **Phase 3** — Plugin `vtherm_auto_fan`    | Implémentation + config flow + service + traductions + tests migrés  | ⬜ À faire  |
| **Phase 4** — Docs                        | README plugin + màj doc VTherm                                       | ⬜ À faire  |

### Détail Phase 1 (livré dans `vtherm_api`)
- ✅ Nouvelle interface `InterfaceFeatureManagerFactory` (`interfaces.py`) : propriété `name`, méthode `create(runtime)`, et méthode `supports(runtime) -> bool` pour le **scope** d'éligibilité (choix retenu : délégation à la factory via `supports`, plutôt qu'un attribut statique).
- ✅ Extension de `InterfaceThermostatRuntime` (`interfaces.py`) avec l'accès fan minimal : `regulated_target_temperature`, `underlying_fan_modes`, `async_set_underlying_fan_mode(fan_mode)` (choix retenu : accès fan fermé/typé plutôt qu'exposition générique des underlyings).
- ✅ Registre distinct dans `VThermAPI` (`vtherm_api.py`) : `register_feature_manager`, `unregister_feature_manager`, `get_feature_manager`, `list_feature_managers`, `get_feature_manager_factories`. Nettoyage dans `reset_vtherm_api`. L'ancien `register_manager` (PluginClimate) est conservé intact.
- ✅ Contrat `InterfaceFeatureManager` confirmé sans changement : `refresh_state` suffit pour piloter le fan à chaque cycle (pas de hook de cycle explicite ajouté pour l'instant).
- ✅ Bump de version `0.3.0` → `0.4.0` (`__init__.py` + export de l'interface, `PKG-INFO`).
- ✅ Tests : nouveau `tests/test_feature_manager_registry.py` (register/unregister/get/list/factories, remplacement, rejet de nom vide, `create`/`supports`, reset). Assertion d'export ajoutée dans `tests/test_package.py`.
- ✅ Documentation API : nouvelle section « Registering a FeatureManager factory (per-thermostat) » dans `README.md`.

### Reste à faire (prochaines étapes)
- **Phase 2 (cœur)** : extraire l'auto-fan (voir §5.1), brancher le point de chargement des factories externes à la construction du thermostat (`get_feature_manager_factories` + `supports` + `create` + `register_manager`), gérer le retry si la factory n'est pas encore enregistrée, et implémenter côté cœur les nouveaux accès runtime (`regulated_target_temperature`, `underlying_fan_modes`, `async_set_underlying_fan_mode`). Relever la contrainte `vtherm_api>=0.4.0`.
- **Phase 3 (plugin)** : créer `vtherm_auto_fan` (voir §6) implémentant `AutoFanManagerFactory` + `AutoFanFeatureManager`.
- **Points ouverts** encore à trancher : migration auto vs manuelle des configs existantes (§7), domaine du service, nom du plugin (§9).
...
- ~~Mécanisme de **scope** de la factory (attribut vs `supports(runtime)`).~~ → **Tranché (Phase 1)** : méthode `supports(runtime) -> bool` sur la factory.
- ~~Étendue de l'extension runtime : underlyings génériques vs accès fan minimal.~~ → **Tranché (Phase 1)** : accès fan minimal et typé (`regulated_target_temperature`, `underlying_fan_modes`, `async_set_underlying_fan_mode`).
- ~~Faut-il un **hook de cycle explicite** dans le contrat `InterfaceFeatureManager`, ou `refresh_state` suffit-il pour piloter le fan à chaque cycle ?~~ → **Tranché (Phase 1)** : `refresh_state` suffit ; pas de hook ajouté pour l'instant.
